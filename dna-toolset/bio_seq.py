...
class bio_seq:
    ...
    def __init__(self, seq = "ATCG", seq_type= "DNA", label = "No Label "): 
        ...
        assert self.is_valid, f"Provided data does not seem to be a correct {self.seq}"

    #DNA tool kit functions 
    def __validate(self):
        """Check sequence to make sure it is a valid DNA string"""
        return  set(NUCLEOTIDE_BASE).issuperset(self.seq )
    
    def get_seq_biotype(self):
        """Returns sequenc e type"""
    ...
    def generate_rnd_seq(self, length =  10, seq_type = "DNA"):
        """Generate a radnom DNA sequence, provided the length"""
    ...
    def gc_content_subsec(self, k = 20):
        ...
        for i in range(0, len(self.seq) - k + 1, k):
            ...
            res.append(round((subseq.count('G') + subseq.count('C')) / len(subseq) * 100))
        return res  
